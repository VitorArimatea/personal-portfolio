  ...
  return (
    <div>
        <button className="relative inline-flex h-12 overflow-hidden rounded-lg p-[1px] focus:outline-none md:w-60 mt-10" onClick={handleClick}>
            ...
            <span className={`inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-[#0a0a0a] dark:bg-[#0A1128] px-6 text-sm font-medium text-white backdrop-blur-3xl ${otherClasses} gap-2.5`}>
                {position == 'left' && icon}
                {title}
                {position == 'right' && icon}
            </span>
